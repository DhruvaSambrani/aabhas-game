...
# ╔═╡ 2663831e-2084-11ed-0726-fd23a9032df7
using Yao

# ╔═╡ 9e87f077-81ca-4aee-aa52-0c4edc2182c6
using PlutoUI

# ╔═╡ 62a0c108-4156-4451-8eb1-c967587e757c
import YaoPlots

# ╔═╡ aebd2393-9d58-4acb-938c-b6b1a8c1df3c
html"<script>present()</script>"

# ╔═╡ 5e5bea26-e7b0-432e-b413-eb6139807a86
C = chain(6)

# ╔═╡ 7b5ccdc4-295e-44d6-8fd1-6e8b32570406
gate_name_mapping = Dict(["H"=>H, "X"=>X, "Y"=>Y])

# ╔═╡ 36d2d6c2-c7a0-404f-8c77-a99c2f29112c
begin
	gates = []
	@bind choice PlutoUI.confirm(PlutoUI.combine() do Child
		md"""
	$(Child(Radio(collect(gate_name_mapping))))
	$(Child(NumberField(1:6)))
	"""
	end
	)
end

# ╔═╡ 080b63cd-1f00-4333-b491-b73853af304d
begin
	choice
	YaoPlots.plot(
	chain(6, map(gates) do gate
		put(Pair(gate[2], gate_name_mapping[gate[1]]))
	end...
	)
	)
end

# ╔═╡ bd6bf1f3-ac67-40f9-a9bd-7815ddaea988
push!(gates, choice);
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "a2b1dcdeca6872c82b10495ce15fd708bde3d072"
...
[[deps.YaoPlots]]
deps = ["Luxor", "Yao"]
git-tree-sha1 = "a7a3a460d1bc9f8e28ef336f3d870a959851c530"
